...
def test_canonical_package_is_explicit_and_bound_to_forward_release():
    ...
    assert registry.package_version == "1.0.7"
    assert len(registry.rules) == 303
    assert registry.unresolved_source_count == 172
    assert payload["source_package_version"] == "2.10.0"
