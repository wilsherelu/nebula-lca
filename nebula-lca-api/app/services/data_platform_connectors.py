...
class TianGongSupabaseConnector(BaseDataPlatformConnector):
    ...
    def _search_rpc(
        ...
        *,
        ...
        logger.info(
            "TianGong %s search endpoint=%s elapsed_ms=%d",
            kind,
            search_rpc,
        ...
        logger.info(
            "TianGong %s search complete endpoint=%s elapsed_ms=%d remote_rows=%d returned_rows=%d remote_total=%s",
            kind,
            search_rpc,
            round((time.perf_counter() - search_started_at) * 1000),
            remote_row_count,
        ...
        exchanges.append(
            {
                ...
                "flow_source_namespace": "tiangong_open_data" if flow is not None and flow.remote_version else None,
                "flow_property_uuid": flow.metadata.get("flow_property_id") if flow is not None and isinstance(flow.metadata, dict) else None,
                "flow_property_version": flow.metadata.get("flow_property_version") if flow is not None and isinstance(flow.metadata, dict) else None,
                "unit_group_uuid": flow.metadata.get("unit_group_id") if flow is not None and isinstance(flow.metadata, dict) else None,
                "unit_group_version": flow.metadata.get("unit_group_version") if flow is not None and isinstance(flow.metadata, dict) else None,
