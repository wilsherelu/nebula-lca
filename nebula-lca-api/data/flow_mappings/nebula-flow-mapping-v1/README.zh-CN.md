...
L1 表示公开 L1 子图内的严格身份映射：每个 TianGong UUID 和 ecoinvent UUID 最多出现一次。L2 表示经审核的兼容映射：每个 TianGong UUID 只选择一个 ecoinvent UUID，但同一个 ecoinvent UUID 可以被多个 TianGong Flow 复用，消费端应建议用户确认。首版不发布 L3 候选。
