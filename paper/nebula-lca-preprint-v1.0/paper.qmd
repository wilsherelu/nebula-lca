...
abstract: |
  Industrial inventory records commonly describe a batch, facility, unit operation, or reporting period and therefore combine total inputs, multiple products, wastes, and direct emissions. Product life cycle assessment (LCA), by contrast, requires an inventory referenced to a selected product and functional demand. This paper presents a traceable compilation framework that separates an unallocated total-process source representation from product-resolved computational representations. Product attribution is expressed through explicit, unit-aware weights and exchange-classification rules. The resulting columns enter a signed technology-system model, $T\mathbf{x}=\mathbf{f}$, with inventory $\mathbf{z}=B\mathbf{x}$ and impact results $\mathbf{h}=C\mathbf{z}$. A product technology system (PTS) is formalized as an internal linear subsystem whose solved activities are projected onto a declared boundary. The implementation is evaluated through five progressively composed, refinery-inspired benchmark projects covering a multi-output unit process, a balanced foreground chain, a normalized electricity supplier, an internal recycle loop, and PTS compilation. All five API-executed audits passed. An independent NumPy oracle reconstructed the classified Case 1 source vector with an infinity-norm residual of $1.39\times10^{-17}$ and rejected a deliberately duplicated product-specific exchange. For the two-process PTS, the oracle reconstructed $A_{\mathrm{pts}}$, $M=I-A_{\mathrm{pts}}$, and the declared boundary and elementary-flow rows; the PTS residual was zero, $\kappa_1(M)=5.0$, and all four nonzero declared components agreed exactly with the compiled artifact. The expanded and PTS-integrated models also returned identical reported EF 3.1 vectors for the tested demand. The benchmarks use hypothetical quantities rather than measured refinery data. Complete ILCD/TIDAS round-trip retention, basis-complete PTS validation, and privacy protection remain outside the demonstrated scope.
...
**Preprint status.** This version reports the formal method and results from five executed, progressively composed benchmark projects plus an independent matrix oracle. The refinery-inspired quantities are hypothetical and are not evidence of representative industrial performance. Complete ILCD/TIDAS round-trip preservation, basis-complete operator equivalence, performance improvement, and privacy protection are not claimed.
...
![The traceable transformation chain separates total-process facts, attribution evidence, product-resolved columns, full-system calculation, and PTS compilation. Solid arrows denote computation; the lower lane denotes provenance continuity.](figures/figure1_method_chain.png){width=6.5in}

\newpage

![Source-state transfer equality and normalized computational closure are different properties. Raw normalized provider and consumer coefficients need not be equal; the activity solution and scaled exchanges provide closure.](figures/figure2_quantity_semantics.png){width=6.5in}
...
![PTS compilation solves the internal dependency system for the exposed-product basis and constructs boundary technosphere and elementary-flow operators.](figures/figure3_pts_operator.png){width=6.5in}
...
![Independent-oracle comparison for the tested PTS reformate column. Panels (a) and (b) compare the declared boundary and elementary-flow components by identity; panel (c) reports component-wise absolute differences. The non-exposed hydrogen by-product is disclosed separately in the evidence table and is outside the shell comparison.](data/figure5_pts_equivalence.pdf){width=6.5in}

## 6.3. Cross-case validation matrix

The five projects are cases in a compositional sequence, whereas the scientific checks cut across that sequence. Table 5 prevents the number of projects from being confused with the number of validation dimensions.

**Table 5. Cross-case validation status.**
...
1. International Organization for Standardization. **ISO 14040:2006. Environmental management - Life cycle assessment - Principles and framework.** 2nd ed. Geneva: ISO; 2006. Confirmed 2022; Amendment 1:2020.
2. International Organization for Standardization. **ISO 14044:2006. Environmental management - Life cycle assessment - Requirements and guidelines.** 1st ed. Geneva: ISO; 2006. Confirmed 2022; Amendments 1:2017 and 2:2020.
...
5. TianGong LCA. **TIDAS: TianGong LCA Data System - public documentation and data contracts.** Commit b10585ba7695ec66636d5078d970ac54105bdf0e. Accessed 31 August 2026.
...
11. Brightway Developers. **Inventory data.** Brightway documentation. Accessed 31 August 2026.
12. Brightway Developers. **Matrix construction.** Brightway documentation. Accessed 31 August 2026.
13. GreenDelta. **Allocation.** openLCA 2 Manual. Accessed 31 August 2026.
14. GreenDelta. **Data aggregation in openLCA.** openLCA 2 Manual. Accessed 31 August 2026.
...
20. Moreau L, Missier P, editors. **PROV-DM: The PROV Data Model.** W3C Recommendation. World Wide Web Consortium; 30 April 2013.
21. Kamali AK, Koyamparambath A, Laratte B, Sonnemann G. Reproducibility as a missing scientific criterion in life cycle assessment: a reporting framework. **International Journal of Life Cycle Assessment.** 2026;31:Article 135. doi:10.1007/s11367-026-02708-y.
...
24. Sonnemann G, Vigon B, editors. **Global Guidance Principles for Life Cycle Assessment Databases: A Basis for Greener Processes and Products.** Paris: UNEP/SETAC Life Cycle Initiative; 2011. ISBN 978-92-807-3174-3.
