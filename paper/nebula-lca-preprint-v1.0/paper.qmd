...
Process aggregation is established in LCA databases and software. The original ecoinvent framework described unit-process and aggregated inventory structures [17], and later database development separated raw data from modeling choices and system models [24]. openLCA distinguishes fully terminated system processes, which expose elementary flows, from partially terminated processes that retain selected intermediate flows for later provider linking [16]. Systematic disaggregation has also been proposed to recover interpretability from aggregated calculations [18].
