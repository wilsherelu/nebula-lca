...
compilation framework that separates an unallocated total-process source
representation from product-resolved computational representations.
Product attribution is expressed through explicit, unit-aware weights
and exchange-classification rules. The resulting columns enter a signed
technology-system model, \(T\mathbf{x}=\mathbf{f}\), with inventory
\(\mathbf{z}=B\mathbf{x}\) and impact results
\(\mathbf{h}=C\mathbf{z}\). A product technology system (PTS) is
formalized as an internal linear subsystem whose solved activities are
projected onto a declared boundary. The implementation is evaluated
...
NumPy oracle reconstructed the classified Case 1 source vector with an
infinity-norm residual of \(1.39\times10^{-17}\) and rejected a
deliberately duplicated product-specific exchange. For the two-process
PTS, the oracle reconstructed \(A_{\mathrm{pts}}\),
\(M=I-A_{\mathrm{pts}}\), and the declared boundary and elementary-flow
rows; the PTS residual was zero, \(\kappa_1(M)=5.0\), and all four
nonzero declared components agreed exactly with the compiled artifact.
The expanded and PTS-integrated models also returned identical reported
EF 3.1 vectors for the tested demand. The benchmarks use hypothetical
quantities rather than measured refinery data. Complete ILCD/TIDAS
round-trip retention, basis-complete PTS validation, and privacy
protection remain outside the demonstrated scope.
\end{abstract}


\setstretch{1.05}
\textbf{Preprint status.} This version reports the formal method and
results from five executed, progressively composed benchmark projects
plus an independent matrix oracle. The refinery-inspired quantities are
hypothetical and are not evidence of representative industrial
performance. Complete ILCD/TIDAS round-trip preservation, basis-complete
operator equivalence, performance improvement, and privacy protection
are not claimed.
...
rules for UUIDs and dataset versions {[}3,4{]}. TIDAS similarly exposes
structured contracts for processes, flows, flow properties, unit groups,
models, and their dependencies {[}5{]}. These specifications are
necessary for data exchange and identity management, but a syntactically
valid package does not determine a multifunctionality choice, guarantee
a stable numerical interpretation after transformation, or prove
equivalence between an expanded network and an aggregated artifact.
...
{\centering \includegraphics[width=6.5in,height=\textheight,keepaspectratio]{figures/figure1_method_chain.png}

}

\caption{The traceable transformation chain separates total-process
facts, attribution evidence, product-resolved columns, full-system
calculation, and PTS compilation. Solid arrows denote computation; the
lower lane denotes provenance continuity.}
...
{\centering \includegraphics[width=6.5in,height=\textheight,keepaspectratio]{figures/figure2_quantity_semantics.png}

}

\caption{Source-state transfer equality and normalized computational
closure are different properties. Raw normalized provider and consumer
coefficients need not be equal; the activity solution and scaled
exchanges provide closure.}
...
{\centering \includegraphics[width=6.5in,height=\textheight,keepaspectratio]{figures/figure3_pts_operator.png}

}

\caption{PTS compilation solves the internal dependency system for the
exposed-product basis and constructs boundary technosphere and
elementary-flow operators.}
...
{\centering \includegraphics[width=6.5in,height=\textheight,keepaspectratio]{figures/figure4_system_boundary.pdf}

}

\caption{System boundary of the public refinery-inspired benchmark. Four
balanced foreground processes are combined with a normalized electricity
supplier; direct elementary flows cross the foreground boundary, whereas
licensed background inventories are not redistributed.}
...
{\centering \includegraphics[width=6.5in,height=\textheight,keepaspectratio]{data/figure5_pts_equivalence.pdf}

}

\caption{Independent-oracle comparison for the tested PTS reformate
column. Panels (a) and (b) compare the declared boundary and
elementary-flow components by identity; panel (c) reports component-wise
absolute differences. The non-exposed hydrogen by-product is disclosed
separately in the evidence table and is outside the shell comparison.}
...
scientific checks cut across that sequence. Table 5 prevents the number
of projects from being confused with the number of validation
dimensions.

\textbf{Table 5. Cross-case validation status.}
...
\item
  ...
  2022; Amendments 1:2017 and 2:2020.
\item
...
\item
  ...
  b10585ba7695ec66636d5078d970ac54105bdf0e. Accessed 31 August 2026.
\item
...
\item
  ...
  documentation. Accessed 31 August 2026.
\item
  GreenDelta. \textbf{Allocation.} openLCA 2 Manual. Accessed 31 August
  2026.
\item
...
\item
  Kamali AK, Koyamparambath A, Laratte B, Sonnemann G. Reproducibility
  as a missing scientific criterion in life cycle assessment: a
  reporting framework. \textbf{International Journal of Life Cycle
  Assessment.} 2026;31:Article 135. doi:10.1007/s11367-026-02708-y.
\item
