...
allocation; total-process inventory; matrix LCA; partially terminated
...
internal subsystem can then be compiled as a partially terminated system
(PTS) that exposes a declared reference product and selected
intermediate exchanges for further modeling without disclosing every
internal process in the main model. The implementation is evaluated
through five progressively composed, refinery-inspired benchmark
projects covering a multi-output unit process, a balanced foreground
chain, a normalized electricity supplier, an internal recycle loop, and
PTS compilation. All five API-executed audits passed. An independent
NumPy calculation reconstructed the classified source inventory to
floating-point precision and rejected a deliberately duplicated
product-specific exchange. For the two-process PTS, the independent
calculation reproduced the internal activity solution and all four
declared nonzero boundary and elementary-flow components exactly; the
expanded and compiled models also returned identical reported EF 3.1
results for the tested demand. The benchmarks use hypothetical
quantities rather than measured refinery data. Complete ILCD/TIDAS
round-trip retention, equivalence over every possible PTS demand, and
privacy protection remain outside the demonstrated scope.
\end{abstract}
...
\textbf{Preprint version.} 1.0, 2 September 2026. DOI:
...
management {[}5{]}. The TianGong database has recently been documented
as an open-source, ILCD entry-level life-cycle unit-process database for
China {[}6{]}, while TIDAS exposes structured contracts for processes,
flows, flow properties, unit groups, models, and their dependencies
{[}7{]}. These specifications are necessary for data exchange and
...
flows and impact indicators {[}8{]}. Multifunctionality has been
...
practical implementation {[}9-11{]}. Existing LCA software supports
...
product nodes {[}12-14{]}, while openLCA supports several allocation
methods and fully or partially terminated system processes {[}15,16{]}.
...
aggregation {[}17-23{]}. The present work therefore does not claim to
...
evidence; and can an internal subsystem be compiled as a partially
terminated system whose declared open exchanges remain linkable while
its boundary quantities, elementary flows, and impacts are demonstrably
the same as those of the expanded subsystem? Can these transformations
retain continuous provenance without assigning source identifiers to
semantically new objects? Finally, can claims be limited by predefined
numerical and semantic oracles rather than by the fact that a software
artifact runs?
...
allocation, and the framing of the decision problem {[}9,10{]}. Economic
...
of negative-value outputs, and price variability {[}11{]}.
...
and dependencies {[}7{]}. These systems support interoperability, but
...
connected by technical exchanges {[}8{]}. Brightway documents several
...
{[}12-14{]}. These representations already support flexible product
...
inventory structures {[}17{]}, and later database development separated
raw data from modeling choices and system models {[}24{]}. ILCD uses the
term \emph{partly terminated system} for an aggregated dataset that
retains at least one non-reference product flow requiring further
modeling {[}3{]}. openLCA uses the equivalent wording \emph{partially
terminated process} for an almost completely aggregated dataset that
retains selected intermediate exchanges for later provider linking
{[}16{]}. The PTS in this paper belongs to this established dataset
class; it is not a synonym for a generic product system. Systematic
disaggregation has also been proposed to recover interpretability from
aggregated calculations {[}18{]}.
...
{[}20{]}. Privacy-preserving aggregation has been studied under explicit
cryptographic and adversarial models {[}21{]}. The PTS artifact
...
expansion are established {[}1,2,9-11,15{]} & Defines a
source-to-product transformation with exchange classification, unit
evidence, closure, and receipt; it does not prescribe a universally
preferred rule. \\
Matrix calculation & Technology-matrix LCA and graph-to-matrix
construction are established {[}8,12-14{]} & Provides consistent
...
aggregation are established {[}16-18{]} & Defines product-specific PTS
...
references {[}3,4,7{]} & Defines a field-level round-trip contract and
...
aggregation have been studied {[}20,21{]} & Defines a declared boundary
...
exist {[}19,22,23{]} & Binds snapshots, matrices, receipts, residuals,
...
Figure 1 summarizes the complete source-to-boundary transformation
chain. The framework then distinguishes source-state quantity auditing
from computational closure (Figure 2).
...
{\centering \includegraphics[width=6.5in,height=\textheight,keepaspectratio]{figures/figure1_method_chain_pptx.png}
...
{\centering \includegraphics[width=6.5in,height=\textheight,keepaspectratio]{figures/figure2_quantity_semantics_pptx.png}
...
entity-activity-agent pattern of W3C PROV {[}22{]}. Source processes,
...
\subsection{4.3. Partially terminated
system}\label{partially-terminated-system}

Following the established ILCD concept of a \emph{partly terminated
system} and the equivalent \emph{partially terminated} terminology used
by openLCA, a partially terminated system (PTS) is an aggregated
process-system representation that deliberately retains one or more
intermediate technosphere exchanges for further modeling {[}3,16{]}. It
differs from a fully terminated system process, whose internal
intermediate exchanges have all been resolved and whose calculated
boundary contains only elementary flows apart from its quantitative
reference.

In the present formalization, a PTS is compiled from a declared internal
subsystem with \(p\) normalized process activities and \(r\) exposed
product interfaces. Its reference product, selected unresolved
technosphere inputs or outputs, wastes, and elementary flows form the
declared external interface; internal technosphere exchanges are solved
and eliminated. Its dependency equation is
...
{\centering \includegraphics[width=6.5in,height=\textheight,keepaspectratio]{figures/figure3_pts_operator_pptx.png}
...
not use a numerically unnecessary explicit inverse {[}25{]}.

\subsection{4.6. Fail-closed conditions}\label{fail-closed-conditions}

The method contract requires compilation or product resolution to fail
closed when any of the following applies:
...
In version 1.0, the duplicated product-specific exchange negative
control was executed independently. The remaining fail-closed conditions
define the implementation contract and are not all represented as
executed benchmark results. A rejected negative case is a successful
validation outcome when rejection is the predeclared oracle. Silent
fallback to names, ``latest'' versions, default ratios, or provider
...
pinned for the test package {[}7{]}. Neither standard is presumed to
...
carbon dioxide is used across the cases. The reported impact vector is
calculated with the European Commission Environmental Footprint 3.1
reference package released in July 2022, as implemented in the pinned
Nebula LCA method package {[}26{]}. The implementation returned 25
ordered indicator records, of which one was nonzero for this
deliberately sparse inventory. These records are regression outputs from
one pinned method set and are not 25 statistically independent
validation dimensions. The LCIA vector is therefore used only as a
propagation and expanded-versus-compiled equivalence check, not as a
comparative environmental assessment of refinery technologies.

\begin{figure}[H]

{\centering \includegraphics[width=6.5in,height=\textheight,keepaspectratio]{figures/figure4_system_boundary_pptx.png}
...
electricity) and the 0.1 kg fossil-carbon-dioxide elementary flow. The
0.0375 kg hydrogen co-product was excluded by the declared PTS interface
projection. The independently expanded oracle applied the same
restricted projection; therefore, the equivalence claim applies only to
this declared interface and does not assert equivalence for a boundary
in which the hydrogen co-product is exposed. The excluded row is
reported separately rather than silently treated as a compiled boundary
...
reformer by-product hydrogen. The declared interface projection marks
this row as non-exposed; the same restricted projection is used for both
the expanded oracle and compiled artifact, so the co-product is reported
separately rather than counted as an agreement row.
...
audits. \\
Limited disclosure & The compiled foreground production chain exposes
three process nodes instead of four, while the external normalized
electricity supplier remains unchanged and is not counted in this
foreground-node comparison; the PTS retains two internal nodes in its
own canvas. & Define disclosed fields, a threat model, target secrets,
and a reconstruction baseline before making any privacy claim. \\
\end{longtable}
}
...
{[}12-14{]}. openLCA already provides allocation choices and
system-process aggregation {[}15,16{]}. ecoinvent has long distinguished
unit-process detail from aggregated or system-model representations
{[}17,24{]}. The contribution here is therefore the explicit semantic
...
between full aggregation and partial termination {[}16{]}. However,
...
cryptographic or secure-aggregation approaches {[}20,21{]}. Until then,
...
datasets, and component-level results {[}23{]}. The case-package design
...
The version 1.0 manuscript and associated reproducibility artifacts are
archived on Zenodo under DOI
\href{https://doi.org/10.5281/zenodo.22244134}{10.5281/zenodo.22244134}.
The archive contains the manuscript source, Supporting Information,
five-case benchmark package, independent validation scripts,
identity-indexed comparison tables, result figures, and SHA-256
manifests. The executed API receipts identify Nebula LCA engine commit
\texttt{4c7cb4b69ae6a0647e3e08d048d6c6724cd93ae3}, the European
Commission Environmental Footprint 3.1 reference package released in
July 2022 as implemented in the pinned Nebula LCA method package, and
the exact elementary-flow identities used by the benchmark. TIDAS
interoperability is referenced to public contract commit
...
from the respective provider under an appropriate license.
...
OpenAI language models, including ChatGPT and Codex, were used under the
author's direction for manuscript restructuring, language editing,
reference-metadata checking, code-to-equation traceability review, and
preparation of the PDF layout. The author reviewed the cited sources,
code evidence, calculations, and final text and accepts full
responsibility for the work. The AI systems are not listed as authors
because they cannot approve the final manuscript or assume
responsibility for its content. No specific funding was declared for
this preprint.
...
\item
  Kamal Kamali A, Koyamparambath A, Laratte B, et al.~Reproducibility as
  a missing scientific criterion in life cycle assessment: a reporting
  framework. \textbf{International Journal of Life Cycle Assessment.}
  2026;31:135. doi:10.1007/s11367-026-02708-y.
\item
...
\item
  Valente A, Kusche O, Ardente F. \textbf{Updates on ``Guide for EF
  compliant data sets (Version 2.0)'' to reflect the changes in the
  Environmental Footprint 3.1 reference package.} European Commission,
